...
function generateReport(out, agg, v) {
  ...
  L.push('');
  L.push('---');
  L.push('数据来源：`.benchmark/phase9_' + (out.runId || 'real') + '.json`');
  return L.join('\n');
}
...
async function main() {
  ...
  const out = { generatedAt: new Date().toISOString(), runId, simulated: false, provider: process.env.AI_PROVIDER, model: process.env.DEEPSEEK_MODEL || 'deepseek-chat', summary: agg, verdict: v, perTask: results };

  const dir = path.resolve(__dirname, '..', '..', '.benchmark');
  try { fs.mkdirSync(dir, { recursive: true }); } catch (e) {}
  fs.writeFileSync(path.join(dir, 'phase9_' + runId + '.json'), JSON.stringify(out, null, 2), 'utf8');
  const reportFile = path.resolve(__dirname, '..', '..', 'PHASE9_REAL_WORLD_RESULT_REPORT.md');
  fs.writeFileSync(reportFile, generateReport(out, agg, v), 'utf8');

  console.log('\n================ PHASE 9 REAL WORLD BENCHMARK ================');
  ...
  console.log('v0.2.0 判定     :', v.v02 ? '✅ 达到候选标准' : '⚠️ 继续 Alpha Hardening');
