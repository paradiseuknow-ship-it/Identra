...
| 评估基准 | 冻结 v2 任务池（100 任务）+ canonical240 基线 + 双回归护栏（178/0 + OK=171/BAD=0） |
...
- **回归护栏**：runRegression **178/0**（403.5s）+ phase9 **OK=171/BAD=0**（C77 后历史最佳；C77 收官 diagnosis/ 深扫，agent/ 子目录扫描空间 C52–C77 全部穷尽；.benchmark 回归日志双落盘）
