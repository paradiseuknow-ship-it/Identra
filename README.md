...
| 评估基准 | 冻结 v2 任务池（100 任务）+ canonical240 基线 + 双回归护栏（179/0 + OK=172/BAD=0） |
...
- **回归护栏**：runRegression **179/0**（330.7s）+ phase9 **OK=172/BAD=0**（C78 后历史最佳；C78 client 面板深扫 batch 3：ObservabilityPanel 时间线 RECOVERY 恢复节点可见性修复 + 渲染契约硬化，.benchmark 回归日志双落盘）
