...
| 评估基准 | 冻结 v2 任务池（100 任务）+ canonical240 基线 + 双回归护栏（177/0 + OK=170/BAD=0） |
...
- **回归护栏**：runRegression **177/0**（356.7s）+ phase9 **OK=170/BAD=0**（C76 后历史最佳；C76 收尾 recovery/+repair/ 深扫，agent/ 子目录扫描空间 C52–C76 全部穷尽；.benchmark 回归日志双落盘）
