...
npm test        # 全量回归（runRegression.js 140 项 + phase9 133 项双护栏）
```
...
| 评估基准 | 冻结 v2 任务池（100 任务）+ canonical240 基线 + 双回归护栏（140/0 + OK=133/BAD=0） |
...
- **回归护栏**：runRegression **140/0** + phase9 **OK=133/BAD=0**（C36 后历史最佳；含 step19 层一致性修复实证全绿）
