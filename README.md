...
| 评估基准 | 冻结 v2 任务池（100 任务）+ canonical240 基线 + 双回归护栏（163/0 + OK=156/BAD=0） |
...
- **回归护栏**：runRegression **163/0**（285.5s）+ phase9 **OK=156/BAD=0**（C62 后历史最佳，工作树口径含 c62 新增套件 1；fp16b launch 套件 profile 目录已随 FPB_DATA_DIR tmp 隔离，消除跨实例 Chrome 锁争用 flaky）
