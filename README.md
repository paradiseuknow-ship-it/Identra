...
| 评估基准 | 冻结 v2 任务池（100 任务）+ canonical240 基线 + 双回归护栏（171/0 + OK=164/BAD=0） |
...
- **回归护栏**：runRegression **171/0**（430.2s）+ phase9 **OK=164/BAD=0**（C69 后历史最佳，工作树口径含 c69 新增套件 1；首跑 phaseD_startup_smoke 启动时序 flake 单独复跑 4/0 后全量复跑干净；fp16b launch 套件 profile 目录已随 FPB_DATA_DIR tmp 隔离，消除跨实例 Chrome 锁争用 flaky）
