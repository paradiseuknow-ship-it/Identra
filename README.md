...
npm test        # 全量回归（runRegression.js 127 项 + phase9 120 项双护栏）
```
...
| 评估基准 | 冻结 v2 任务池（100 任务）+ canonical240 基线 + 双回归护栏（127/0 + OK=120/BAD=0） |
...
- **回归护栏**：runRegression **127/0**（243s）+ phase9 **OK=120/BAD=0**（C23 后历史最佳）
