...
| 评估基准 | 冻结 v2 任务池（100 任务）+ canonical240 基线 + 双回归护栏（172/0 + OK=166/BAD=0） |
...
- **回归护栏**：runRegression **172/0**（340.1s）+ phase9 **OK=166/BAD=0**（C71 后历史最佳，工作树口径含 c71 新增套件 1；首跑 test_step22_business_e2e 启动时序 flake 单独复跑 63/0 后全量复跑干净——C69/C70/C71 三度实证同族；fp16b launch 套件 profile 目录已随 FPB_DATA_DIR tmp 隔离，消除跨实例 Chrome 锁争用 flaky）
